...
llama_kv_cache::llama_kv_cache(
    ...
    if (reuse) {
        ...
        for (uint32_t il = 0; il < n_layer; il++) {
            ...
            // questions, so the filter alone does not decide here: a layer with no KV of its own
            // still keeps the entry it borrows, and a layer the filter rejected with KV of its own
            // does too. Only a layer that is both filtered out and has no KV is skipped.
